...
const EmployeeView: FC<EmployeeViewProps> = ({ 
  ...
  // Box component for consistent styling with darker text
    ...
    <div className="bg-white fixed inset-0 z-50 w-full h-full overflow-y-auto">
      ...
      <div className="p-6">
        ...
        <div className="bg-gray-50 rounded-lg p-6 mb-6 flex flex-col md:flex-row items-center md:items-start gap-6">
          <div className="w-32 h-32 rounded-full bg-blue-700 text-white flex items-center justify-center text-4xl font-bold shrink-0">
            {initials}
          </div>
          <div className="text-center md:text-left">
            <h3 className="text-2xl font-bold text-gray-900">{fullName}</h3>
            <p className="text-gray-700 font-medium mb-1">{employee.jobRole}</p>
            <p className="text-sm text-gray-600 mb-4">{employee.department || 'Department not specified'}</p>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-700">{employee.mobileNumber}</span>
              </div>
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                <span className="text-sm font-medium text-gray-700">{employee.location || 'Location not specified'}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <InfoBox title="Skills">
              <div className="flex flex-wrap gap-2">
                {employee.skills.map((skill, index) => (
                  <span 
                    key={index} 
                    className="bg-blue-100 text-blue-800 font-medium px-3 py-1 rounded-full text-sm"
                  >
            ...
            <InfoBox title="Languages">
              <div className="flex flex-wrap gap-2">
                {employee.languages.map((language, index) => (
                  <span 
                    key={index} 
                    className="bg-green-100 text-green-800 font-medium px-3 py-1 rounded-full text-sm"
                  >
                    {language}
                  </span>
                ))}
