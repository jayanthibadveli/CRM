...
import { Icons, navigationItems } from '@/app/dashboard/manager/utilis/index';
import "./globals.css";
...
function Sidebar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-gradient-to-b from-indigo-800 to-indigo-900 text-white transition-all duration-300 ease-in-out`}>
        <div className="flex items-center justify-between p-4 border-b border-indigo-700">
          <div className={`flex items-center ${!sidebarOpen && 'justify-center w-full'}`}>
            <div className="flex-shrink-0">
              <div className="h-8 w-8 bg-white rounded-md flex items-center justify-center">
                <span className="text-indigo-800 font-bold">G</span>
              </div>
            </div>
            {sidebarOpen && (
          ...
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className="text-white hover:bg-indigo-700 p-1 rounded">
              ...
              <li key={item.id}>
                ...
                >
                  <div className={`${!sidebarOpen && 'mx-auto'} ${
                    activeItem === item.id ? 'text-white' : 'text-indigo-200'
                  }`}>{item.icon}</div>
                  {sidebarOpen && (
                    <span className={`ml-3 ${
                  ...
                  {sidebarOpen && activeItem === item.id && (
                    <span className="ml-auto bg-white rounded-full p-1">
                      <span className="text-indigo-600 h-3 w-3">
                        <Icons.Check />
                      </span>
                    </span>
                  )}
